...
function Attendances(): React.ReactElement {
  ...
  const showFlashMessage = (message: string, type: string = 'success'): void => {
    ...
    if (showCreateModal || showEditModal) {
      ...
      if (token && companyId) {
        axios.get(`/api/patients?company_id=${companyId}`, { headers: { Authorization: `Bearer ${token}` } })
          .then(res => {
            const sorted = (res.data || []).sort((a: Patient, b: Patient) => a.name.localeCompare(b.name));
            setPatients(sorted);
          });
        axios.get(`/api/places?company_id=${companyId}`, { headers: { Authorization: `Bearer ${token}` } })
          .then(res => {
            const sorted = (res.data || []).sort((a: Place, b: Place) => a.name.localeCompare(b.name));
            setPlaces(sorted);
          });
        axios.get(`/api/users?company_id=${companyId}&limit=1000`, { headers: { Authorization: `Bearer ${token}` } })
          .then(res => {
            // Filtrar apenas DOCTOR
            const doctors = (res.data.users || []).filter((u: User) => u.role === 'DOCTOR');
            const sorted = doctors.sort((a: User, b: User) => a.name.localeCompare(b.name));
            setUsers(sorted);
          });
      }
    }
  ...
  const handleShowEdit = async (attendanceId: number): Promise<void> => {
    try {
  ...
  const handleShowDelete = (attendanceId: number): void => {
  ...
  const handleFormSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const url = isEditing ? `/api/attendances/${(formData as any).id}` : '/api/attendances';
  ...
  const handleFinishAttendance = async (attendanceId: number): Promise<void> => { //deixei exclusivo para doctor
    try {
  ...
  const handleStartConsultation = async (attendanceId: number): Promise<void> => { // deixei exclusivo para doctor
    try {
      ...
      setAllAttendances(prevAttendances =>
        prevAttendances.map(attendance =>
          attendance.id === attendanceId
            ? { ...attendance, confirmed_at: currentTime.toISOString() } as any
            : attendance
        )
      );
            ...
            <tbody>
              {attendancesParaExibir.map(a => (
                <tr key={a.id}>
                  ...
                  <td>
                    {(a as any).confirmed_at ? (
                      new Date((a as any).confirmed_at).toLocaleString('pt-BR')
                    ) : (
