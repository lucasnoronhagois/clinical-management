...
function Users() {
  const [users, setUsers] = useState([]); // filtrados
  const [allUsers, setAllUsers] = useState([]); // todos
  ...
  useEffect(() => {
    if (allUsers.length > 0) {
      const filtrados = allUsers.filter(user => {
        const nome = removerAcentos(user.name?.toLowerCase() || '');
        const busca = removerAcentos(filters.name.toLowerCase());
        return (
          (filters.name === '' || nome.includes(busca)) &&
  ...
  const handleShowEdit = async (userId) => {
    try {
      const token = localStorage.getItem('token');
