...
function Patients(): React.ReactElement {
    ...
    if (showCreateModal || showEditModal) {
      ...
      if (token) {
        axios.get('http://localhost:3000/api/companies', { headers: { Authorization: `Bearer ${token}` } })
          .then(res => {
            const sorted = (res.data || []).sort((a: Company, b: Company) => a.name.localeCompare(b.name));
            setCompanies(sorted);
          });
      }
    }
  ...
  const handleShowEdit = async (patientId: number): Promise<void> => {
    try {
      const token = localStorage.getItem('token');
  ...
  const handleShowDelete = (patientId: number): void => {
  ...
  const handleFormSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const endpoint = isEditing ? `/api/patients/${(formData as any).id}` : '/api/patients';
      const method = isEditing ? 'put' : 'post';
      
      // garantir que o company_id está sendo enviado mesmo que não apareça em tela
      const dataToSend = {
        ...formData,
        company_id: parseInt(formData.company_id || selectedCompanyId || '0')
      };
      ...
      if (err.response?.data?.details) {
        const errorDetails = err.response.data.details.map((detail: any) => 
